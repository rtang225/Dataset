...
for idx, row in orig_df.iterrows():
    ...
    if len(week_data) > 0:
        ...
        feature_cols = [c for c in week_data.columns if c in ['wind_speed_10m', 'soil_moisture_0_to_7cm', 'soil_moisture_7_to_28cm', 'wind_gusts_10m', 'soil_moisture_28_to_100cm', 'soil_moisture_0_to_100cm', 'soil_temperature_0_to_100cm', 'soil_temperature_28_to_100cm']]
...
np.save('week_sequences_largefires.npy', np.array(sequences, dtype=object), allow_pickle=True)
np.save('week_targets_largefires.npy', np.array(targets), allow_pickle=True)
