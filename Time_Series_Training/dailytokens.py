...
# Add temporal tokens (time of day embedding)
def get_time_of_day_embedding(seq_len, embedding_dim=8):
    # Assume each timestep is an hour in a day (0-23)
    hours = np.arange(seq_len) % 24
    # Normalize to [0, 1]
    hours_norm = hours / 23.0
    # Sin/cos encoding
    sin_enc = np.sin(2 * np.pi * hours_norm)
    cos_enc = np.cos(2 * np.pi * hours_norm)
    # Stack and tile to match embedding_dim
    emb = np.stack([sin_enc, cos_enc], axis=1)
    emb = np.tile(emb, (1, embedding_dim // 2))
...
class SimpleTransformerClassifier(nn.Module):
    def __init__(self, input_size, num_classes, seq_len, d_model=128, nhead=4, num_layers=2, dim_feedforward=512, dropout=0.1):
        ...
        self.time_of_day_embedding = nn.Parameter(get_time_of_day_embedding(seq_len, embedding_dim=8).unsqueeze(0), requires_grad=False)
        self.time_proj = nn.Linear(8, d_model)
    ...
    def forward(self, x, mask=None):
        ...
        x = x + self.time_proj(self.time_of_day_embedding[:, :x.size(1), :].to(x.device))
