...
# Only keep samples with class 1, 2, 3
keep_indices = np.where((target_classes > 0) & (target_classes <= 3))[0]
sequences = [sequences[i] for i in keep_indices]
targets = targets[keep_indices]
target_classes = target_classes[keep_indices]
...
criterion = FocalLoss(alpha=[1, 2, 1.25, 1.25], gamma=3, reduction='mean')
