...
# Load the dataset
file_path = 'fp-historical-wildfire-data.csv'
df = pd.read_csv(file_path)
df = df.drop(columns=['RESPONSIBLE_GROUP', 'ACTIVITY_CLASS', 'TRUE_CAUSE', 'DISTANCE_FROM_WATER_SOURCE', 'FIRE_START_DATE', 'YEAR', 'SIZE_CLASS', 'LATITUDE', 'LONGITUDE'])

# Remove only the top 5% of area values as outliers
upper_bound = df['CURRENT_SIZE'].quantile(0.99)
df = df[df['CURRENT_SIZE'] <= upper_bound]

# Assume 'CURRENT_SIZE' is the target column
features = [col for col in df.columns if col != 'CURRENT_SIZE']
...
for col in features:
    ...
    sns.scatterplot(x=df[col], y=df['CURRENT_SIZE'])
    plt.xlabel(col)
